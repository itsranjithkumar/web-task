...
export default function ComposedChart({
  ...
  useEffect(() => {
    ...
    const datasets = yAxis.map((axis) => {
      const chartType = axis.chart
      const dataKey = axis.title
      ...
      if (chartType === "area") {
        ...
        if (dataKey.includes("body_battery")) {
          dataset.borderColor = "#8b5cf6"
          dataset.backgroundColor = "rgba(139, 92, 246, 0.2)"
        } else if (dataKey.includes("strain")) {
          dataset.borderColor = "#ef4444"
          dataset.backgroundColor = "rgba(239, 68, 68, 0.2)"
        } else if (dataKey.includes("recovery")) {
          dataset.borderColor = "#10b981"
          dataset.backgroundColor = "rgba(16, 185, 129, 0.2)"
        } else if (dataKey.includes("total_strain")) {
        ...
        if (dataKey.includes("strain")) {
          dataset.backgroundColor = "rgba(239, 68, 68, 0.8)"
        } else if (dataKey.includes("recovery")) {
          dataset.backgroundColor = "rgba(16, 185, 129, 0.8)"
        } else if (dataKey.includes("physical")) {
          dataset.backgroundColor = "rgba(59, 130, 246, 0.8)"
        } else if (dataKey.includes("sedentary")) {
          dataset.backgroundColor = "rgba(245, 158, 11, 0.8)"
        } else {
          dataset.backgroundColor = "rgba(139, 92, 246, 0.8)"
        }
    ...
    chartInstance.current = new Chart(ctx, {
      type: "bar", // Default type, will be overridden by dataset type
      data: {
        labels,
        datasets,
      },
